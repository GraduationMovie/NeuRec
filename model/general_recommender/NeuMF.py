...
class NeuMF(AbstractRecommender):
    ...
    def _create_placeholders(self):
        with tf.name_scope("input_data"):
            self.user_input = tf.placeholder(
                tf.int32, shape=[None], name='user_input')
            self.item_input = tf.placeholder(
                tf.int32, shape=[None], name='item_input')
            if self.is_pairwise is True:
                self.item_input_neg = tf.placeholder(
                    tf.int32, shape=[None], name="item_input_neg")
            else:
    ...
    def _create_variables(self, params=None):
        with tf.name_scope("embedding"):  # The embedding initialization is unknown now
            if params is None:
                ...
                self.mf_embedding_user = tf.Variable(
                    params[0][0], name='mf_embedding_user', dtype=tf.float32)
                self.mf_embedding_item = tf.Variable(
                    params[0][1], name='mf_embedding_item', dtype=tf.float32)

                self.mlp_embedding_user = tf.Variable(
                    params[1][0], name="mlp_embedding_user", dtype=tf.float32)
                self.mlp_embedding_item = tf.Variable(
                    params[1][1], name="mlp_embedding_item", dtype=tf.float32)

    def _create_inference(self, item_input):
        with tf.name_scope("inference"):

            mf_user_latent = tf.nn.embedding_lookup(
                self.mf_embedding_user, self.user_input)
            mf_item_latent = tf.nn.embedding_lookup(
                self.mf_embedding_item, item_input)
            mlp_user_latent = tf.nn.embedding_lookup(
                self.mlp_embedding_user, self.user_input)
            mlp_item_latent = tf.nn.embedding_lookup(
                self.mlp_embedding_item, item_input)
            ...
            for idx in np.arange(len(self.layers)):
                mlp_vector = tf.layers.dense(
                    mlp_vector, units=self.layers[idx], activation=tf.nn.relu)

            # Concatenate MF and MLP parts
            predict = tf.reduce_sum(
                tf.concat([mf_vector, mlp_vector], axis=1), 1)
            return mf_user_latent, mf_item_latent, mlp_user_latent, mlp_item_latent, predict

    def _create_loss(self):
        with tf.name_scope("loss"):
            p1, q1, m1, n1, self.output = self._create_inference(
                self.item_input)
            if self.is_pairwise is True:
                _, q2, _, n2, output_neg = self._create_inference(
                    self.item_input_neg)
    ...
    def _create_optimizer(self):
        with tf.name_scope("learner"):
            self.optimizer = learner.optimizer(
                self.learner, self.loss, self.learning_rate)
    ...
    def train_model(self):
        ...
        if self.is_pairwise is True:
            data_iter = PairwiseSampler(
                self.dataset, neg_num=1, batch_size=self.batch_size, shuffle=True)
        else:
            data_iter = PointwiseSampler(
                self.dataset, neg_num=self.num_negatives, batch_size=self.batch_size, shuffle=True)
        ...
        if candidate_items_user_ids is not None:
            ...
            for u in user_ids:
                ...
                feed_dict = {self.user_input: users,
                             self.item_input: np.arange(self.num_items)}
                ratings.append(self.sess.run(self.output, feed_dict=feed_dict))
        return ratings
